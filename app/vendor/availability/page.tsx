    ...
    <div className="mx-auto max-w-xl space-y-6">
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Override your regular schedule for specific dates. Add extra availability or block time windows when you&apos;re not available.</p>
        </div>
        ...
        {customTimes.length > 0 ? (
          ...
          <p className="text-xs text-gray-400">No date overrides set.</p>
        )}

        <div className="border-t border-gray-200 pt-4 space-y-3">
          {/* Mode toggle */}
          <div>
            <label className="mb-1.5 block text-xs text-gray-500">What do you want to do?</label>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => setNewCustomMode('block')}
                className={`flex-1 rounded-lg border-2 px-3 py-2.5 text-sm font-medium transition-all ${newCustomMode === 'block' ? 'border-red-400 bg-red-50 text-red-700' : 'border-gray-200 text-gray-500 hover:border-gray-300'}`}
              >
                Block time
                <span className="block text-[10px] font-normal mt-0.5">{newCustomMode === 'block' ? 'Remove slots from this window' : ''}</span>
              </button>
              <button
                type="button"
                onClick={() => setNewCustomMode('add')}
                className={`flex-1 rounded-lg border-2 px-3 py-2.5 text-sm font-medium transition-all ${newCustomMode === 'add' ? 'border-green-400 bg-green-50 text-green-700' : 'border-gray-200 text-gray-500 hover:border-gray-300'}`}
              >
                Add extra time
                <span className="block text-[10px] font-normal mt-0.5">{newCustomMode === 'add' ? 'Open slots beyond regular hours' : ''}</span>
              </button>
            </div>
          </div>
        ...
        <div className="rounded-lg border border-blue-100 bg-blue-50 p-3 text-xs text-blue-800">
          <strong>Example:</strong> Your regular hours are 9 AM – 5 PM, but you&apos;re leaving at 2 PM today? Block 2:00 PM – 5:00 PM.
          Need to work late on Saturday? Add extra time 5:00 PM – 8:00 PM.
        </div>
      </div>
    </div>
  );
